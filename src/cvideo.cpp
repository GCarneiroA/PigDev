...
int CVideo::DecodeThread(void* pUserData){
...
int CVideo::QueuePicture(AVFrame *frame, double pts){
...
double CVideo::SyncVideo(AVFrame *frame, double pts){
    double frameDelay;
...
int CVideo::VideoThread(void *pUserData){
    CVideo *player = (CVideo*)pUserData;
    VideoState *is = player->is;
    int rv;
    double pts;
...
int CVideo::DecodeAudioFrame(double *ptsPtr){
...
void CVideo::AudioCallback(void *userdata, uint8_t *stream, int len){
...
int CVideo::DecodeInterruptCallback(void *userData){
    CVideo *player = (CVideo*)userData;
    return player->is && player->quit;
}

int CVideo::StreamVideoComponentOpen(){
...
int CVideo::StreamAudioComponentOpen(){
    AVFormatContext *pFormatCtx = is->pFormatCtx;
...
CVideo::CVideo(std::string nomeArq,int idJanela)
    : CVisual(CGerenciadorJanelas::GetJanela(idJanela)->GetAltura(),CGerenciadorJanelas::GetJanela(idJanela)->GetLargura(),nomeArq,idJanela)
{
    volume = 0.5;
...
void CVideo::SetSeek(double incremento){
    if (!is) return;
...
void CVideo::Stop(){
    ...
    if (estado==VIDEO_PARADO) return;//Stop() pode ser chamado isoladamente ou pelo destrutor, mas n�o deve ser executado 2 vezes
    estado = VIDEO_PARADO;
    quit = 1;
...
void CVideo::SetVolume(double valor){
    volume = valor;
    if (volume<0) volume = 0;
}

double CVideo::GetVolume(){
    return volume;
}

double CVideo::GetTempoAtual(){
    return is->videoClock;
}

std::string CVideo::GetTempoAtualString(){
...
double CVideo::GetTempoTotal(){
    return (is->pFormatCtx->duration*1.0/AV_TIME_BASE);
}

std::string CVideo::GetTempoTotalString(){
...
/*
void CVideo::SetAngulo(float a){
    angulo = a;
}

double CVideo::GetAngulo(){
    return angulo;
}

void CVideo::SetFlip(PIG_Flip valor){
    flip = valor;
}

PIG_Flip CVideo::GetFlip(){
    return flip;
}

void CVideo::SetPivo(int px,int py){
    pivoRelativo.x = px;
    pivoRelativo.y = destVideo.h-py;
    //printf("pivo mudado para %d,%d\n",pivoRelativo.x,pivoRelativo.y);
}

void CVideo::SetPivo(float px,float py){
    pivoRelativo.x = px*destVideo.w;
    pivoRelativo.y = destVideo.h-py*destVideo.h;
}

void CVideo::GetPivo(int &px,int &py){
    px = pivoRelativo.x;
    py = pivoRelativo.y;
}
*/

void CVideo::Move(int nx,int ny){
    CVisual::Move(nx,ny);
    janelaToda = false;
}

void CVideo::SetDimensoes(int altura,int largura){
    CVisual::SetDimensoes(altura,largura);
    janelaToda = false;
}

void CVideo::OcupaJanelaInteira(){
    angulo = 0;
    x = y = 0;
    dest.x = 0;
...
void CVideo::UsaResolucaoOriginal(){
    janelaToda = false;
}
