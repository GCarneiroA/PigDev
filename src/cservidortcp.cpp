...
CServidorTCP::CServidorTCP(int idSocket, int maximoConexoes, int porta, int maxBytesPacote)
    : CSocketTCP(idSocket,"",porta,maxBytesPacote)
{
    if (!ativo){
    ...
    clientesTCP = (CSocketTCP**) malloc(maxConexoes*sizeof(CSocketTCP));
...
int CServidorTCP::GetPortaLocal(){
    return portaLocal;
}

int CServidorTCP::GetMaximoConexoes(){
    return maxConexoes;
}

int CServidorTCP::GetQuantidadeConexoes(){
    return qtdConexoes;
}

int CServidorTCP::EnviaDados(const void *buffer,int tamanhoBuffer,int indiceSlot){
    if (!clientesTCP) return -1;
    if (!clientesTCP[indiceSlot]) return -2;
    return clientesTCP[indiceSlot]->EnviaDados(buffer,tamanhoBuffer);
}

int CServidorTCP::accept_code(void *data){
    ...
    while (servidor->qtdConexoes>=0){//sinaliza��o de encerramento da thread
        //printf("Checando...\n");
        ...
        if (pronto>0){
            ...
            if (servidor->qtdConexoes==servidor->maxConexoes){
                CSocketTCP *clienteTemp = new CSocketTCP(-99999,servidor->socket,servidor->tamPacote,servidor->clienteSet);
...
int CServidorTCP::receive_code(void *data){
    CServidorTCP *servidor = (CServidorTCP*)data;
    if (servidor==NULL){
...
void CServidorTCP::CriaEventoMensagem(PIG_TipoMensagemRede tipoMensagem, const void *buffer, int tamanhoDados, int indiceSlot){
...
void CServidorTCP::RegistraBytesRecebidos(int qtdBytes){
    if (!ativo) return;
...
void CServidorTCP::RegistraBytesEnviados(int qtdBytes){
    if (!ativo) return;
...
void CServidorTCP::InicializaValoresBasicos(int valorId,int maxBytesPacote){
