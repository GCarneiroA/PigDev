...
#include "ctimer.h"
    ...
    CJogo::CJogo(char *nome,int cursor,int altura,int largura){
        rodando = true;
    ...
    void CJogo::DefineFuncaoPintarArea(PIG_Cor (*funcao)(int,int,int,int,PIG_Cor,PIG_Cor)){
        if (offRenderer)
            offRenderer->DefineFuncaoPintarArea(funcao);
    }

     PIG_Teclado CJogo::PegaTeclado(){
        return teclado;
    }
    ...
    void CJogo::IniciaDesenho(int idJanela){
        CGerenciadorJanelas::IniciaDesenho(idJanela);
    }

    void CJogo::EncerraDesenho(int idJanela){
    ...
    void CJogo::PreparaOffScreenRenderer(int altura,int largura){
     ...
     void CJogo::SalvaOffScreenPNG(char *nomeArquivo){
        if (offRenderer)
            offRenderer->SalvarImagemPNG(nomeArquivo);
    }

     int CJogo::GetXCaneta(){
        if (offRenderer)
            return offRenderer->GetXCaneta();
        return 0;
    }

     int CJogo::GetYCaneta(){
        if (offRenderer)
            return offRenderer->GetYCaneta();
        return 0;
    }

     void CJogo::PintaAreaOffScreen(int px,int py,PIG_Cor cor,void *ponteiro){
        if (offRenderer)
            offRenderer->PintarArea(px,py,cor,ponteiro);
    }

     void CJogo::PintarPoligono(int px[],int py[],int lados,PIG_Cor cor,int idJanela){
        ...
        COffscreenRenderer *off = new COffscreenRenderer(alt,larg); //ajustado extamente com a altura e largura
     ...
     void CJogo::PintaFundoOffScreen(PIG_Cor cor){
        if (offRenderer)
            offRenderer->PintarFundo(cor);
    }

     void CJogo::DesenhaRetanguloOffScreen(int x1,int y1,int altura,int largura,PIG_Cor cor){
        if (offRenderer)
            offRenderer->DesenharRetangulo(x1,y1,altura,largura,cor);
    }

     void CJogo::DesenhaRetanguloVazadoOffScreen(int x1,int y1,int altura,int largura,PIG_Cor cor){
        if (offRenderer)
            offRenderer->DesenharRetanguloVazado(x1,y1,altura,largura,cor);
    }

     void CJogo::DesenhaLinhaOffScreen(int x1,int y1,int x2,int y2,PIG_Cor cor){
        if (offRenderer)
            offRenderer->DesenharLinha(x1,y1,x2,y2,cor);
    }

     void CJogo::MoveCanetaOffscreen(double nx,double ny){
        if (offRenderer)
            offRenderer->MoveCanetaPara(nx,ny);
    }

     void CJogo::AvancaCanetaOffscreen(double distancia){
        if (offRenderer)
            offRenderer->AvancaCaneta(distancia);
    }

     void CJogo::MudaCorCanetaOffscreen(PIG_Cor novaCor){
        if (offRenderer)
            offRenderer->MudaCorAtualCaneta(novaCor);
    }

     void CJogo::GiraCanetaHorarioOffscreen(double angulo){
        if (offRenderer)
            offRenderer->GiraCanetaHorario(angulo);
    }

     void CJogo::GiraCanetaAntiHorarioOffscreen(double angulo){
        if (offRenderer)
            offRenderer->GiraCanetaAntiHorario(angulo);
    }

     void CJogo::GiraCanetaAnguloFixoOffscreen(double angulo){
        if (offRenderer)
            offRenderer->GiraAnguloFixo(angulo);
    }

     double CJogo::GetAngAtual(){
        if (offRenderer)
