...
CJanela::CJanela(std::string tituloJanela,int idJanela,int altTela,int largTela){
    id = idJanela;
    titulo = tituloJanela;
    altura = altTela;
    largura = largTela;
...
SDL_Window *CJanela::GetWindow(){
    return window;
}

SDL_Renderer *CJanela::GetRenderer(){
    return renderer;
}
...
int CJanela::GetFechada(){
    return fechada;
}

void CJanela::Esconde(){
...
void CJanela::GanhaFoco(){
    SDL_RaiseWindow(window);
}

void CJanela::DefineFundo(std::string nomeArquivo){
...
void CJanela::SaveScreenshot(std::string nomeArquivo, bool BMP) {
    if (window==NULL) return;
...
void CJanela::EncerraDesenho(){
    SDL_RenderPresent(renderer);

}

int CJanela::GetAltura(){
    return altura;
}

int CJanela::GetLargura(){
    return largura;
}

std::string CJanela::GetTitulo(){
    return titulo;
}

void CJanela::SetTitulo(std::string novoTitulo){
    if (window==NULL) return;
    titulo = novoTitulo;
    SDL_SetWindowTitle(window,titulo.c_str());
}

PIG_Cor CJanela::GetCorFundo(){
    return corFundo;
}

void CJanela::SetCorFundo(PIG_Cor cor){
    corFundo = cor;
}

float CJanela::GetOpacidade(){
    return opacidade;
}

void CJanela::SetOpacidade(float valor){
    opacidade = valor;
    SDL_SetWindowOpacity(window,opacidade);
}

void CJanela::SetPosicao(int x,int y){
    if (window==NULL) return;
    SDL_SetWindowPosition(window,x,y);
}

void CJanela::GetPosicao(int *x,int *y){
    *x = px;
    *y = py;
}

void CJanela::SetBorda(int valor){
    if (window==NULL) return;
    SDL_SetWindowBordered(window,SDL_bool(valor));
}

void CJanela::SetModo(int valor){
    if (window==NULL) return;
    modo = valor;
...
int CJanela::SetTamanho(int alt, int larg){
    ...
    void CJanela::DesenhaRetangulo(int x, int y, int alturaRet, int larguraRet, PIG_Cor cor){
        SDL_Rect rect;
        rect.x = x;
    ...
    void CJanela::DesenhaRetanguloVazado(int x, int y, int alturaRet, int larguraRet, PIG_Cor cor){
        SDL_Rect rect;
        rect.x = x;
        rect.y = altura-(y+alturaRet);
        rect.h = alturaRet;
    ...
    void CJanela::DesenhaLinhaSimples(int x1,int y1,int x2,int y2,PIG_Cor cor){
        SDL_SetRenderDrawColor(renderer,cor.r,cor.g,cor.b,255);
        SDL_RenderDrawLine(renderer,x1,altura-y1-1,x2,altura-y2-1);
    }

    void CJanela::DesenhaLinhasDisjuntas(int *x,int *y,int qtd,PIG_Cor cor){
        SDL_SetRenderDrawColor(renderer,cor.r,cor.g,cor.b,255);
    ...
    void CJanela::DesenhaLinhasSequencia(int *x,int *y,int qtd,PIG_Cor cor){
        SDL_SetRenderDrawColor(renderer,cor.r,cor.g,cor.b,255);
    ...
    PIG_Cor CJanela::GetPixel(int x,int y) {
