...
#include <SDL2/SDL_image.h>
    ...
    SDL_Renderer *COffscreenRenderer::GetRenderer(int layer){
        return layers[layer].render;
    }

    SDL_Surface *COffscreenRenderer::GetSurface(int layer){
    ...
    int COffscreenRenderer::GetDepth(){
        return depth;
    }

    void COffscreenRenderer::SetCorTransparente(int layer, bool transparencia, PIG_Cor cor){
        SDL_SetColorKey(layers[layer].surf,transparencia,SDL_MapRGBA(layers[layer].surf->format,cor.r,cor.g,cor.b,cor.a));
    }

    void COffscreenRenderer::MergeSurface(int layerSuperior, int layerInferior, SDL_BlendMode modo){
    ...
    Layer COffscreenRenderer::CriaLayer(){
        Layer l;
    ...
    void COffscreenRenderer::LimpaLayer(int layer, PIG_Cor cor){
    ...
    COffscreenRenderer::COffscreenRenderer(int altura,int largura,int qtdMaxCamadas){
        alt = altura;
        larg=largura;
    ...
    void COffscreenRenderer::DesenhaCirculoFinal(int raio, PIG_Cor corFundo, PIG_Cor corCirculo, double angInicial,double angFinal,int layer){
        int xm = larg/2;
        int ym = alt/2;
    ...
    void COffscreenRenderer::DesenhaCirculoFatia(int centroX,int centroY,int raio, PIG_Cor cor, double angInicial, double angFinal,SDL_Point &iniP,SDL_Point &fimP,int layer){
...
/*
    SDL_Point COffscreenRenderer::DesenhaCirculo2(int centroX,int centroY,int raio, PIG_Cor cor, double angFinal,int layer){
    ...
    void COffscreenRenderer::DesenhaCirculo(int raioInterno, int raioExterno, PIG_Cor cor, double angFinal,bool horario,int layer){
        if (angFinal==0) return;
...
*/
    void COffscreenRenderer::PintarFundo(SDL_Color cor,int layer){
        SDL_SetRenderDrawColor(layers[layer].render,cor.r,cor.g,cor.b,cor.a);
        SDL_RenderClear( layers[layer].render );
    }

    void COffscreenRenderer::PintarArea(int px,int py,PIG_Cor cor,void *ponteiro,int layer){
    ...
    void COffscreenRenderer::DesenharLinha(int x1,int y1,int x2,int y2,PIG_Cor cor,int layer){
        SDL_SetRenderDrawColor(layers[layer].render,cor.r,cor.g,cor.b,cor.a);
        SDL_RenderDrawLine(layers[layer].render,x1,alt-y1-1,x2,alt-y2-1);
    }

    void COffscreenRenderer::DesenharRetangulo(int x,int y,int altura,int largura,PIG_Cor cor,int layer){
    ...
    void COffscreenRenderer::DesenharRetanguloVazado(int x,int y,int altura,int largura,PIG_Cor cor,int layer){
        SDL_Rect r={x,alt-y-altura-1,largura,altura};
    ...
    void COffscreenRenderer::CarregaPixelsSurface(int layer){
        SDL_RenderReadPixels(layers[layer].render, NULL, SDL_PIXELFORMAT_ARGB8888, layers[layer].surf->pixels, layers[layer].surf->pitch);
    }

    void COffscreenRenderer::SalvarImagemBMP(char *nomearq,int layer){
        SDL_SaveBMP(layers[layer].surf, nomearq);
    }

    void COffscreenRenderer::SalvarImagemPNG(char *nomearq,int layer){
        IMG_SavePNG(layers[layer].surf, nomearq);
    }

    void COffscreenRenderer::DefineFuncaoPintarArea(PIG_Cor (*funcao)(int, int, int, int, PIG_Cor,PIG_Cor)){
        userFunctionPintarArea = funcao;
    }

    void COffscreenRenderer::MoveCanetaPara(double nx,double ny){
        px = nx;
        py = ny;
    }

    void COffscreenRenderer::GiraCanetaHorario(double angulo){
        ang -= angulo;
    }

    void COffscreenRenderer::GiraCanetaAntiHorario(double angulo){
        ang += angulo;
    }

    void COffscreenRenderer::GiraAnguloFixo(double angulo){
        ang = angulo;
    }

    double COffscreenRenderer::GetAngAtual(){
        return ang;
    }

    void COffscreenRenderer::AvancaCaneta(double distancia,int layer){
    ...
    void COffscreenRenderer::MudaCorAtualCaneta(PIG_Cor novaCor){
        corAtual = novaCor;
    }

    double COffscreenRenderer::GetXCaneta(){
        return px;
    }

    double COffscreenRenderer::GetYCaneta(){
        return py;
    }
