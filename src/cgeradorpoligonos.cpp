...
CGeradorPoligono::CGeradorPoligono()
{
...
CGeradorPoligono::~CGeradorPoligono()
{
    ...
    void CGeradorPoligono::GeraPoligonoRegular(int raio,int lados,PIG_Cor cor){
        ...
        //off->EncerrarDesenho();
        off->SalvarImagemBMP((char*)"poli.bmp");
    ...
    void CGeradorPoligono::GeraRetanguloChanfrado(int altura,int largura,int chanfro,PIG_Cor cor){
        ...
        off->PintarArea(centroX+1,centroY+1,cor);
        //off->EncerrarDesenho();
        off->SalvarImagemBMP((char*)"chanfro.bmp");

        delete off;
    }

    void CGeradorPoligono::GeraEstrela(int raio,int pontas,PIG_Cor cor){
        int lados=2*pontas;
        off = new COffscreenRenderer(raio*2+3,raio*2+3);
        off->PintarFundo(PRETO);
        int centroX = raio+1;
        ...
        off->PintarArea(centroX,centroY,cor);
        //off->EncerrarDesenho();
        off->SalvarImagemBMP((char*)"poli2.bmp");
    ...
    void CGeradorPoligono::GeraCirculoAberto(int raio,int angulo,PIG_Cor cor){
        off = new COffscreenRenderer(raio*2+3,raio*2+3);
        off->PintarFundo(PRETO);
        int centroX = raio+1;
        int centroY = raio+1;
