...
CClienteTCP::CClienteTCP(int idSocket, std::string host, int porta, int maxBytesPacote)
    : CSocketTCP(idSocket, host, porta, maxBytesPacote)
{
    if (!ativo) {
...
CClienteTCP::~CClienteTCP()
{
    ativo = false;
    SDL_Delay(20);//esperar a thread encerrar
}

int CClienteTCP::thread_code(void *data)
{
...
void CClienteTCP::CriaEventoMensagem(PIG_TipoMensagemRede tipoMensagem, const void *buffer, int tamanhoDados)
{
